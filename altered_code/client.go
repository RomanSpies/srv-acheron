...
func (c *client) updateNowPlaying(ctx context.Context, sessionKey string, info ScrobbleInfo, currentuserId string) error {
	params := url.Values{}
	...
	go func() { 
		...
		customParams := map[string]interface{}{
            ...
            "artist": info.artist,
            "album":  info.album,
			"tracklength": info.duration,
			"user": currentuserId,
        }

        jsonData, err := json.Marshal(customParams)
        if err != nil {
            log.Error(ctx, "Failed to marshal custom POST data", "error", err)
            return
        }

        req, err := http.NewRequest(http.MethodPost, "http://URL/scrobbleUpdate", bytes.NewBuffer(jsonData))
        if err != nil {
            log.Error(ctx, "Failed to create custom POST request", "error", err)
            return
        }
        ...
        if err != nil {
            log.Error(ctx, "Failed to send custom POST request", "error", err)
            return
        }
        defer resp.Body.Close()

        if resp.StatusCode != http.StatusOK {
            log.Warn(ctx, "Custom POST request returned non-OK status", "status", resp.Status)
        }
		log.Info(ctx, "Custom parameters marshalled to JSON", "json", string(jsonData))
    }()
	return nil
}
