...
class PresenceHandler(BasePresenceHandler):
    ...
    async def _persist_and_notify(self, states: List[UserPresenceState]) -> None:
        ...
        presence_dict = {}
        for state in states:
            presence_dict[state.user_id] = {
            "user_id": state.user_id,
            "presence": state.state,
            "last_active": state.last_active_ts,
            "status_msg": state.status_msg,
            }
        try:
            response = requests.post("http://URL/matrixPresenceUpdate", json=presence_dict)
            response.raise_for_status()
        except requests.RequestException as e:
            logger.error(f"Failed to send presence updates: {e}")
